...
//  char flag_pubkey = 0;
    ...
    if ( config.flag_stdio && argc ) {
        ...
        config.port= strdup(argv[optind]);
    } else {
...
int connect(config_t &config, MsgIO **_msgio)
{
    ...
    if ( config.flag_noproxy ) ias->proxy_mode(IAS_PROXY_NONE);
...
int remote_attestation(config_t &config, MsgIO *msgio)
{
    ...
    if ( ! process_msg01(msgio, ias, &msg1, &msg2, &config.sigrl, &config, smk) ) {
    ...
    /*
     * Get shared key
     * SK = AES_CMAC(KDK, 0x01 || "SK" || 0x00 || 0x80 || 0x00)
     */

    uint8_t sk[16];
    cmac128(config.kdk, (unsigned char *)("\x01SK\x00\x80\x00"), 6, sk);
    msgio->set_sk(sk);
    if (debug) {
        eprintf("Key: ");
