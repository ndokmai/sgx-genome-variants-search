...
int do_attestation (sgx_enclave_id_t eid, config_t *config, MsgIO **_msgio);
...
int remote_attestation(config_t& config, MsgIO **_msgio)
{
    ...
    if ( config.mode == MODE_ATTEST ) {
        do_attestation(eid, &config, _msgio);
...
int do_attestation (sgx_enclave_id_t eid, config_t *config, MsgIO **_msgio)
{
    ...
    // derive secret key and keep in enclave
    enclave_derive_key(eid, ra_ctx);
