...
#define MSGIO_BUFFER_SZ	16*1024*1024
...
public:
	...
	void disconnect();
        ...
        void set_sk(uint8_t sk[16]);
	int read(void **dest, size_t *sz);
	int read_bin(void **dest, size_t *sz);

	void send_partial(void *buf, size_t f_size);
	void send(void *buf, size_t f_size);
        void send_bin(void *buf, size_t f_size);
        void send_bin_encrypted(void *buf, size_t fsize);
};
