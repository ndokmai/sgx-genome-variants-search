...
int svdcomp_a(float **a, int m, int n, float *w, float **v) {
	int flag, i, its, j, jj, k, l, nm;
	double c, f, h, s, x, y, z;
	double anorm = 0.0, g = 0.0, scale = 0.0;
	double *rv1;
	...
	for (i = 0; i < n; i++) {
		...
		if (i < m) {
			...
			if (scale) {
				...
				if (i != n - 1) {
					for (j = l; j < n; j++) {
						for (s = 0.0, k = i; k < m; k++)
							s += ((double) a[i][k] * (double) a[j][k]);
						f = s / h;
						for (k = i; k < m; k++)
							a[j][k] += (float) (f * (double) a[i][k]);
					}
				}
				for (k = i; k < m; k++)
					a[i][k] = (float) ((double) a[i][k] * scale);
		...
		if (i < m && i != n - 1) {
			...
			if (scale) {
				...
				a[l][i] = (float) (f - g);
				for (k = l; k < n; k++)
					rv1[k] = (double) a[k][i] / h;
				if (i != m - 1) {
					for (j = l; j < m; j++) {
						for (s = 0.0, k = l; k < n; k++)
							s += ((double) a[k][j] * (double) a[k][i]);
						for (k = l; k < n; k++)
							a[k][j] += (float) (s * rv1[k]);
					}
				}
	...
	for (i = n - 1; i >= 0; i--) {
		if (i < n - 1) {
			if (g) {
				for (j = l; j < n; j++)
					v[i][j] =
							(float) (((double) a[j][i] / (double) a[l][i]) / g);
				/* double division to avoid underflow */
				for (j = l; j < n; j++) {
					for (s = 0.0, k = l; k < n; k++)
						s += ((double) a[k][i] * (double) v[j][k]);
					for (k = l; k < n; k++)
						v[j][k] += (float) (s * (double) v[i][k]);
				}
			}
		...
		for (its = 0; its < 30; its++) { /* loop over allowed iterations */
			...
			for (j = l; j <= nm; j++) {
				...
				for (jj = 0; jj < m; jj++) {
					y = (double) a[j][jj];
					z = (double) a[i][jj];
					a[j][jj] = (float) (y * c + z * s);
					a[i][jj] = (float) (z * c - y * s);
				}
			}
			rv1[l] = 0.0;
			rv1[k] = f;
